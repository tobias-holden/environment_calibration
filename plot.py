...
def plot_convergence(bo, ymin = None, ymax = None, negate=False, title="", objective = IdentityMCObjective()):
    ...
    ax.scatter(np.arange(0, n), oY * sign, color='black', linewidth=1, marker='o', label="Sample", size=1)
...
def plot_X_flat(bo, skip = 0, negate=True, param_key = None, labels = None):
    ...
    for i in range(skip, len(X)):
        ...
        if y < 0:
            y = -1*y
        converted_X.append(translate_parameters(param_key,X[i],i)["emod_value"])
        if y > best:
            best = y
            bestx.append(translate_parameters(param_key,X[i],i)["emod_value"])
            besty.append(y)
            besti.append(i)
