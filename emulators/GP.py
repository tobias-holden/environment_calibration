...
class ExactMultiTaskGP(GP):
    ...
    def fit(self, X, Y):
        ...
        # As a word of caution, we've found that when fitting the HOGP model
        # using first-order optimizers (e.g. Adam) as is used in fit_gpytorch_torch 
        # tends to outperform second-order optimizers such as L-BFGS-B due to 
        # the large number of free parameters in the HOGP. L-BFGS-B tends to overfit in practice here.
        # Find optimal model hyperparameters
        fit_gpytorch_torch(mll, options={"maxiter": 3000, "lr": 0.1, "disp": False})
        # fit_gpytorch_torch(mll, options={"maxiter": 3000, "lr": 0.1, "disp": True})
