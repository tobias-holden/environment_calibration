...
tp %>%
  ...
  geom_point(data=.%>%filter(param_set %in% imps),
             aes(alpha=param_set %in% all_scores$param_set[complete.cases(all_scores)],
                color="improvement",shape=param_set==best$param_set),fill="white") +
  theme_minimal(base_size=16) +
...
tp %>%
  ...
  mutate(unit_value=as.numeric(unit_value)) %>%
  mutate(param_set=param_set+batch_size*as.numeric(round)) %>%
  select(param_set,parameter,transformation,emod_value,unit_value) -> TP

scores %>% left_join(TP) %>% 
  gather("score_type","score",c("eir_score","intensity_score","prevalence_score","shape_score")) %>%
  ggplot(aes(x=unit_value,y=score)) +
  facet_wrap(score_type~parameter,scales="free_y") +
  geom_point(aes(color=param_set)) +
...
testing %>%
  ...
  group_by(Year,agebin,Sample_ID) %>%  
  mutate(max_inc=max(inc)) %>%
  ungroup() %>% 
  mutate(norm_inc=inc/max_inc) %>% 
  group_by(month) %>% summarize(norm_inc=mean(norm_inc)) %>%
  ggplot(aes(x=month,y=norm_inc)) +
  geom_line()
```
