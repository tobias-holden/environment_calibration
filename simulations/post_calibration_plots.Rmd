...
scores %>% left_join(TP) %>% 
  select(-c(emod_value,transformation))%>%
...
for(r in seq(0,max(scores$round),1)){
...
spread_df %>% 
  ...
  spread_df %>%
    filter(round<=r)%>%
    ggplot(aes(x=Temperature_Shift,y=CONST_Multiplier))  +
    facet_wrap(~score_type,scales="free_y") +
    geom_point(aes(color=log(score))) +
    scale_color_distiller(palette="Spectral") +
    theme_minimal(base_size=12) +
    theme(legend.position="right") -> pp3

  spread_df %>%
    filter(round<=r)%>%
    ggplot(aes(x=TEMPR_Multiplier,y=WATEV_Multiplier))  +
    facet_wrap(~score_type,scales="free_y") +
    geom_point(aes(color=log(score))) +
    scale_color_distiller(palette="Spectral") +
    theme_minimal(base_size=12) +
    theme(legend.position="right") -> pp4
